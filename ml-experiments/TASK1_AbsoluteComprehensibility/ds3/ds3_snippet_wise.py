...
class DS3_SNIPPET_WISE:
    ...
    def __init__(self, helper, ds):
        self.helper = helper
        self.csv_data_dict = DS3_SNIPPET_WISE.csv_data_dict
        self.ds = ds
    ...
    def evaluate(self, model, X_test, y_test, target, task):
        ...
        classifi_report = classification_report(y_test, y_pred, output_dict=True,
                                                zero_division=0)  ## this ensures that the f1 score is 0 if there are no predicted instances
        ...
        baseline_f1_1, baseline_f1_2, baseline_f1_3, baseline_f1_4, baseline_f1_5, baseline_f1_weighted = self.getBestBaselineModel(
            target)

        if len(present_classes) == 2:
            ## roc_auc_score for the positive class (i.e probability of the class with the greater label, i.e the index 1)
            filtered_y_pred_proba = np.nan_to_num(filtered_y_pred_proba, nan=0)
        ...
        f1_weighted = f1_score(y_test, y_pred, average="weighted", labels=present_classes,
                               zero_division=0)  ## zero_division=0 to avoid nan values to include in the average
